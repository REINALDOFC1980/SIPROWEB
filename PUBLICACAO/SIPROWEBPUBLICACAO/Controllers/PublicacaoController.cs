...
namespace SIPROWEBPUBLICACAO.Controllers
{
    ...
    public class PublicacaoController : Controller
    {
        ...
        public async Task<List<PublicacaoDOMModel>> GerarDOM(string lote)
        {
            ...
            var apiUrl = $"{_baseApiUrl}publicacao/gerar-dom/{lote}";
            var response = await _httpClient.GetAsync(apiUrl);
        ...
        public async Task<IActionResult> GerarWord(string lote)
        {
            ...
            List<PublicacaoDOMModel> resultado = await GerarDOM("32025");
            ...
            using (MemoryStream memStream = new MemoryStream())
            {
                using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(memStream, WordprocessingDocumentType.Document, true))
                {
                    ...
                    foreach (var item in lista)
                    {
                        ...
                        row.Append(WordHelper.CreateCell(item[0], false, 4800, 16, true));
                        row.Append(WordHelper.CreateCell(item[1], false, 1600, 16, true));
                        row.Append(WordHelper.CreateCell(item[2], false, 1500, 16, true));
                        row.Append(WordHelper.CreateCell(item[3], false, 1700, 16, true));
                        tabela.Append(row);
                    }
